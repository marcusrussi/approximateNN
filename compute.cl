...
__kernel void add_rows_step_0(const size_t height, const size_t len,
			      ...
			      __global float *r) {
  size_t x = get_global_id(0), y = get_global_id(1);
  float g = len & !x? a[(len - 1) * height + y] : 0;
  r[x * height + y] = a[x * height + y] + a[(x + len / 2) * height + y] + g;  
}
...
__kernel void add_rows_step_n(const size_t height, const size_t len,
			      __global float *r) {
  size_t x = get_global_id(0), y = get_global_id(1);
  float g = !x && len % 2 ? r[(len - 1) * height + y] : 0;
...
__kernel void divide_by_length(const size_t length, __global float *r) {
...
__kernel void apply_rotation(const size_t height,
			     ...
			     __global const float *ang,
			     __global float *a) {
  size_t x = get_global_id(0) * height, y = get_global_id(1);
  size_t k = i[y], l = j[y];
  float c;
  float s = sincos(ang[y], &c);
  float q = a[x + k] * c - a[x + l] * s;
  float r = a[x + k] * s + a[x + l] * c;
  a[x + k] = q;
  a[x + l] = r;
}
...
__kernel void apply_permutation(const size_t height_pre,
				...
				__global float *r) {
  size_t x = get_global_id(0), y = get_global_id(1);
  float g = perm[y] < height_pre? a[x * height_pre + perm[y]] : 0;
  r[x * height_post + y] = g;
}
...
__kernel void apply_walsh(const size_t lheight,
  ...
  case 1:
    {
      float a1 = a[x << 1] + a[x << 1 | 1];
      float a2 = a[x << 1] - a[x << 1 | 1];
      a[x << 1] = a1 * rsr;
      a[x << 1 | 1] = a2 * rsr;
      return;
    }
  ...
  for(size_t step = 0, s = 1; step < lheight; step++, s <<= 1) {
    for(int j = 0; j < 8; j++) {
      ...
      size_t yl = y1 ^ yh;
      size_t ya = yh << 1 | yl;
      float alpha = a[x << lheight | ya], beta = a[x << lheight | ya | s];
...
__kernel void compute_diffs_squared(const size_t dims,
				    ...
				    __global const float *pointsa,
				    __global const float *points,
				    __global float *diff_results) {
...
__kernel void add_cols(const size_t height,
  ...
  size_t j = k - skip;
  size_t x = get_global_id(0), y = get_global_id(1), z = get_global_id(2);
  float g = height & !z? mat[(x * j + y + 1) * height - 1] : 0;
  mat[(x * j + y) * height + z] +=
  ...
  for(size_t s = height >> 2, os = (height>>1); s > 0; os = s, s >>= 1) {
    ...
    float h = os & !z? mat[(x * j + y) * height + s * 2] : 0;
...
__kernel void add_cols_step(const size_t height,
			    ...
			    __global float *mat) {
  size_t x = get_global_id(0), y = get_global_id(1), z = get_global_id(2);
  float g = s & !z? mat[(x * k + y) * height + s - 1] : 0;
...
__kernel void sort_two_step(const size_t count,
  ...
  for(int i = 0; i < 8; i++) {
    ...
    if(count > y_b) {
      size_t alpha = x + y_a;
      size_t beta = x + y_b;
      float ao = order[alpha], bo = order[beta];
      size_t aa = along[alpha], ba = along[beta];
      size_t doswap = -(ao > bo); // minimize divergence.
...
// Note: long is the same length as float, we're doing bit tricks here!
