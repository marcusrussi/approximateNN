...
size_t *MK_NAME(precomp) (size_t n, size_t k, size_t d, const double *points,
  ...
  for(int i = 0; i < tries; i++) {
    ...
    for(size_t j = 0; j < rots_before; j++) {
    	  rand_rot(rot_len_before, d,
		   ri + j, rj + j, ra + j);
	  rot_is_b[j] =
	    MK_BUF_COPY_RO_NA(gpu_context, size_t, rot_len_before, ri);
	  rot_js_b[j] =
	    MK_BUF_COPY_RO_NA(gpu_context, size_t, rot_len_before, rj);
	  rot_as_b[j] =
	    MK_BUF_COPY_RO_NA(gpu_context, double, rot_len_before, ra);
    }
    free(ri);
    free(rj);
    free(ra);
    ...
    for(size_t j = 0; j < rots_after; j++) {
    	  rand_rot(rot_len_after, d,
		   ri + j, rj + j, ra + j);
	  rot_is_b[j] =
	    MK_BUF_COPY_RO_NA(gpu_context, size_t, rot_len_after, ri);
	  rot_js_b[j] =
	    MK_BUF_COPY_RO_NA(gpu_context, size_t, rot_len_after, rj);
	  rot_as_b[j] =
	    MK_BUF_COPY_RO_NA(gpu_context, double, rot_len_after, ra);
    }
    free(ri);
    free(rj);
    free(ra);
    ...
    relMem(diffs);
    DoSort(q, (d_short + 1) * tmax, n, which_d, dists);
  ...
  DoSort(q, k * tries, n, pointers_out, dists_out);
  ...
  enqueueCopy2D(q, size_t, k * tries, 0, k * (k + 1), pointers_out, nedge,
		n, k);
  relMem(pointers_out);
  BUFTYPE(double) ndists = MK_BUF_RW_NA(gpu_context, double,
					n * k * (k + 1) + 1);
  enqueueCopy2D(q, double, k * tries, 0, k * (k + 1), dists_out, ndists, n, k);
  relMem(dists_out);
...
size_t *MK_NAME(query) (const save_t *save, const double *points,
  ...
  AddUpCols(q, save->d_long, msofar * (save->d_short + 1), 0, ycnt,
	      diffs, dpts);
  relMem(diffs);
  DoSort(q, msofar * (save->d_short + 1), ycnt, ipts, dpts);
  ...
  AddUpCols(q, save->d_long, save->k * (save->k + 1), save->k, ycnt,
	      diffs, dpts);
  relMem(diffs);
  DoSort(q, save->k * (save->k + 1), ycnt, ipts, dpts);
