...
static void add_up_rows(size_t d, size_t n, float *points, float *sums) {
  LOOP2(, add_rows_step_0(d, n, points, sums), n/2, d);
  for(size_t m = n >> 1; m >> 1; m >>= 1)
    LOOP2(, add_rows_step_n(d, m, sums), m/2, d);
}

static void walsh(size_t d, size_t n, float *a) {
  if(d == 1)
    return;
  int l = lg(d);
...
static void do_sort(size_t k, size_t n, size_t *along, float *order) {
  int lk = lg(k);
